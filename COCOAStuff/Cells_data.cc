...
void Cells_data::set_tree_branches(TTree *outTree)
{
	...
	outTree->Branch("cell_id", "vector<int>", &cell_id);
	outTree->Branch("cell_ilay", "vector<int>", &cell_ilay);
	outTree->Branch("cell_ieta", "vector<int>", &cell_ieta);
	outTree->Branch("cell_iphi", "vector<int>", &cell_iphi);
	...
	for (int ilay = 0; ilay < ilay_num; ilay++)
	{
	  for (int ieta = 0; ieta < (int) Number_Pixel_Flatten.at(ilay); ieta++)
	  {
	      for (int iphi = 0; iphi < (int) Number_Pixel_Flatten.at(ilay); iphi++)
	      {
		    ...
		    cell_id.push_back(cell_counter);
		    cell_ilay.push_back(ilay);
		    cell_ieta.push_back(ieta);
		    cell_iphi.push_back(iphi);
